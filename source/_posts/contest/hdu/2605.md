...
//            if (top.step - 2 > dp[top.x][top.y][top.st]) continue;
            ...
            for (int d = 0; d < 4; ++d) {
                ...
                // 吃过豆, 并且上一个豆到当前点距离
                int lastX = beanX[top.lastBeanId];
                int lastY = beanY[top.lastBeanId];
                if (nSt != 0 && dp[lastX][lastY][top.st][top.lastBeanId] + beanToAnywhere[top.lastBeanId][nx][ny] + L < dp[nx][ny][nSt][nLastBeanId]) {
                    go = true;
                }
